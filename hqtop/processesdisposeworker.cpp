...
void ProcessesDisposeWorker::sortProcesses(QList<ProcessInfo> processesWaitSort,int column,bool isMsgBox,
                   Qt::SortOrder order)
{
    // 根据列索引定义排序规则
    switch (column) {
    case 0:  // PID 列
        isMsgBox = false;
        std::sort(processesWaitSort.begin(), processesWaitSort.end(), [order](const ProcessInfo &a, const ProcessInfo &b)
        {
